...
const InicioConductor = () => {
  ...
  const [latitudActual, setLatitudActual] = React.useState(0);
  const [longitudActual, setLongitudActual] = React.useState(0);
  ...
  React.useEffect(() => {
    if (latitude && longitude) {
      setLatitudActual(latitude);
      setLongitudActual(longitude);
    }
    if (error) {
    ...
    getOrdenesByIdConductor((state && state.idConductor) ? state.idConductor : parseInt(localStorage.getItem('idConductor')))
      .then(function (response) {
        let array = [];
        let arrayFull = [];
        let despachando = response.data.filter((element) => element.estadoOrden.idEstadoOrden === 2);
        if(despachando.length !== 0){
          array.push({
            idOrden: despachando[0].idOrdenRecojo,
            cliente: despachando[0].sedeCliente.cliente.razonSocial,
            producto: despachando[0].productoVenta.producto.nombre,
            marca: despachando[0].productoVenta.marca.nombre,
            unidad: despachando[0].productoVenta.unidad.nombre,
            cantidad: despachando[0].cantidad,
            estado: despachando[0].estadoOrden.idEstadoOrden
          });
          arrayFull.push(despachando[0]);
          setEstaDespachando(1);
        }
        else {
          ...
          <Grid>
            ...
            <Typography className='modal-text-error'>
              {messageError.map((mes, index) => {
                return (
                  <div key={index} style={{ marginBottom: "5px" }}>{index + 1}. {" " + mes + "\n"}</div>
                )
              })}
            </Typography>
          </Grid>
